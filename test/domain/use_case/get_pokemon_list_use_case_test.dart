...
void main() {
  ...
  setUp(() {
    useCase = GetPokemonListUseCaseImpl(repository: repository);
  });

  const successData = [PokemonListItem(name: "name", id: "1")];
  ...
  test(
    ...
    verifyNoMoreInteractions(repository);
    expect(result,
        equals(UseCaseResult<List<PokemonListItem>>.genericFailure(error)));
  });
  ...
  test(
    ...
    verifyNoMoreInteractions(repository);
    expect(result,
        equals(UseCaseResult<List<PokemonListItem>>.serverFailure(error)));
  });
}
